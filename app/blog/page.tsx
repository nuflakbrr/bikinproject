...
export default async function BlogIndexPage() {
  const blogs = (await getAllBlogs()).sort(
    (a, b) =>
      stringToDate(b.frontmatter.date).getTime() - stringToDate(a.frontmatter.date).getTime(),
  );
  return (
...
function BlogCard({ date, title, description, slug }: BlogMdxFrontmatter & { slug: string }) {
  return (
    <div className="flex flex-col md:flex-row items-start">
      ...
      <div className="md:border-l md:pl-14 pb-12 relative">
        ...
        <Link className="flex flex-col gap-3" href={`/blog/${slug}`}>
          ...
          <Button variant="link" size="sm" className="w-fit px-0 underline -mt-2">
